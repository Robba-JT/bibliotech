    ...
    document.setEvents("DOMContentLoaded", function () {
        ...
        var arraymove = function (arr, fromIndex, toIndex) {
            ...
            Bookcell = function (book, indice) {
                var totalCells = Bookcells.cells.length + (typeof indice === "undefined" ? 0 : indice),
                ...
                this.cell.setAttribute("bookid", JSON.stringify(book.id));
            ...
            Bookcells = {
                ...
                bytags: function (tag) {
                    ...
                    µ.one("#selectedTag").html(tag).parentNode.toggle(true);
                    Bookcells.order(tag);
            ...
            Detail = {
                action: function () {
                    ...
                    this.close = function () {
                        ...
                        this.closest("window").fade(false).then(function (elt) {
                            delete Windows.on;
                            if (!!elt && !!elt.one("iframe") && !!window.frames.iPreview && !!window.frames.iPreview.document.getElementById("viewer")) {
                ...
                uploadCover: function () {
                    ...
                    if (!!image[0]) {
                        if (!image[0].type.match(/image.*/) || image[0].size > 100000) {
                            Windows.confirm("error", "Veuillez sélectionner un fichier de type \"image\" inférieure à 100KB.");
                            return false;
                        }
            ...
            Dock = {
                ...
                drop: function (event) {
                    ...
                    if (!!µ.one(".description")) { µ.one(".description").removeAll(); }
                    if (!!target) {
                        if (cell !== target) {
                            target.closest("col").insertBefore(cell, target);
                            Bookcells.loadcovers();
                            if (µ.one("#collection").hasClass("active") && !!µ.one("#selectedTag").html() && !µ.one("@filtre").value) { µ.one("#saveorder").toggle(true); }
                            if (µ.one(".sortBy")) { Images.blur.call(µ.one(".sortBy").toggleClass("sortBy", false)); }
                        }
                    } else {
                        this.appendChild(cell);
                        if (µ.one(".sortBy")) { Images.blur.call(µ.one(".sortBy").toggleClass("sortBy", false)); }
                        if (µ.one("#collection").hasClass("active") && !µ.one("@filtre").value) { µ.one("#saveorder").toggle(true); }
                    }
                    return false;
                },
                get: function () {
            ...
            Search = {
                ...
                clear: function () {
                    ...
                    µ.one("#formFilter").reset();
                    if (µ.one(".sortBy")) { Images.blur.call(µ.one(".sortBy").toggleClass("sortBy", false)); }
                    µ.one("#selectedTag").text("").parentNode.toggle(false);
                    µ.one("#saveorder").toggle(false);
                },
            ...
            shortCuts = function (event) {
                ...
                if (!event.altKey) {
                    if (!event.ctrlKey) { if (event.keyCode === 27) { Windows.close(); Tags.close(); µ.one("#contextMenu").fade(false); }} else {
                        var action;
                        if ([77, 76, 82, 80, 66, 69, 73, 72].indexOf(event.keyCode) !== -1 && µ.one("#wa").isVisible()) { action = true; } else {
                        ...
                        connection
                            ...
                            this.once("disconnect", function (data) {
                                ...
                                Windows.close().then(function () {
                                    ...
                                    µ.one("#selectedTag").html("").parentNode.toggle(false);
                                    ...
                                    for (var jta = 0, lg = covers.length; jta < lg; jta++) {
                                        ...
                                        if (!bookcell.book) {
                                            ...
                                            if (!!bookcell.cell) {
                                                ...
                                                if (!!Detail.data.book && Detail.data.book.id === cover.id) { µ.one("#detailCover").src = cover.base64 || cover.alternative; }
                                            }
                                        }
                                    }
                                    resolve();
                    ...
                    return new Promise(function (resolve) {
                        if (µ.one("#wa").isVisible() || !µ.one("#collection").hasClass("active")) { resolve(); } else {
                            Windows.close().then(function () {
                                ...
                                cloud.toggle(!vis);
                                resolve();
                                if (!vis && !cloud.alls("span").length) { Tags.generate(); }
                            });
                        }
                    });
                }
            ...
            userActions = {
                ...
                initCollect: function (part) {
                    ...
                    return new Promise(function (resolve) {
                        Waiting.anim(true);
                        if (!part || !part.length) { resolve(); } else { Bookcells.show(part).then(resolve); }
                    });
                },
                collection: function (books) {
                    return new Promise(function (resolve) {
                        ...
                        µ.one("#selectedTag").parentNode.toggle(false);
                        µ.one("#saveorder").toggle(false);
                        if (!µ.one("#collection").hasClass("active")) {
            ...
            Waiting = {
                anim: function (toShow) {
                    if (µ.one("#wa").isVisible() !== toShow) {
                        µ.one("#wa").fade(toShow);
                        µ.one("#nvb").toggleClass("inactive", toShow);
                    }
                },
            ...
            var action = function () {
                ...
                description = function (event) {
                    ...
                    if (!self.book.description) { return; } else {
                        ...
                        setTimeout(function () { description.setEvents("mouseleave", leave).fade(true); }, 1000);
                    }
                },
                destroy = function () {
                ...
                detail = function () {
                    var isCollect = User.get().bookindex(self.id) !== -1;
                    if (!!isCollect || !!self.opened) {
                        Detail.data = self;
                        Detail.show(!!isCollect);
                    } else {
